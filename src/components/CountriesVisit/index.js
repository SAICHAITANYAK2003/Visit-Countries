...
class CountriesVisit extends Component {
  ...
  updateVisit = id => {
    this.setState(prevState => {
      const updatedCountriesList = prevState.countriesList.map(eachCountry => {
      ...
      const visitedCountry = updatedCountriesList.find(
  ...
  updateRemoveItem = id => {
    this.setState(prevState => {
      ...
      const updatedVisitedCountriesList = prevState.visitedCountriesList.filter(
        eachCountry => eachCountry.id !== id,
      )
  ...
  renderNotVisited = () => (
    <EmptyContainer>
      <EmptyTitle>No Countries Visited Yet</EmptyTitle>
    </EmptyContainer>
  )
  ...
  render() {
    ...
    const visitedCountriesListLength = visitedCountriesList.length > 0

    const sortedVisitedCountriesList = [...visitedCountriesList].sort((a, b) =>
      a.name.localeCompare(b.name),
    )
    return (
      <VisitCountriesContainer>
        <CountiriesTitle>Countries</CountiriesTitle>
        ...
        <CountriesListContainer>
          {countriesList.map(eachCountry => (
            <CountryItem
              ...
              isVisit={eachCountry.isVisited}
            />
          ))}
        </CountriesListContainer>

        <VisitedCountriesContainer>
          <VisitedCountriesTitle> Visited Countries</VisitedCountriesTitle>
          {visitedCountriesListLength ? (
            <VisitedCountriesList>
              {sortedVisitedCountriesList.map(eachVisitedCountry => (
                <VisitedCountryItem
                  eachVisitedCountry={eachVisitedCountry}
                  key={eachVisitedCountry.id}
                  updateRemoveItem={this.updateRemoveItem}
                />
              ))}
            </VisitedCountriesList>
          ) : (
